...
def get_app_60_active_days_sql():
    # 任务配置
    project_id = "oppo-gcp-prod-digfood-129869"
    tag_name = "'App_60_active_days'"
    ...
    partition_last_60_date = "'{{ execution_date.add(days=-58).strftime('%Y-%m-%d') }}'"
    ...
    from (
        ...
        where partition_date between DATE({partition_last_60_date}) and DATE({partition_date})
