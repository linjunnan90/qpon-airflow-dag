...
def get_app_7_active_days_sql():
    # 任务配置
    project_id = "oppo-gcp-prod-digfood-129869"
    tag_name = "'App_7_active_days'"
    ...
    partition_last_7_date = "'{{ execution_date.add(days=-5).strftime('%Y-%m-%d') }}'"
    ...
    from (
        ...
        where partition_date between DATE({partition_last_7_date}) and DATE({partition_date})
