...
def get_app_15_active_days_sql():
    # 任务配置
    project_id = "oppo-gcp-prod-digfood-129869"
    tag_name = "'App_15_active_days'"
    ...
    partition_last_15_date = "'{{ execution_date.add(days=-13).strftime('%Y-%m-%d') }}'"
    ...
    from (
        ...
        where partition_date between DATE({partition_last_15_date}) and DATE({partition_date})
